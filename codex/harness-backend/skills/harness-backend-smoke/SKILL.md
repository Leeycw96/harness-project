...
description: 读 .harness/call-chain/{slug}.md,逐入口生成 URL + 请求 JSON 报文给用户用 Postman 测试。非 HTTP 入口可在用户确认后添加临时 /_smoke/trigger/{name} HTTP 入口；skill 只生成与引导,不执行业务请求。
---
...
本 skill 不写 shell 脚本、不发任何业务请求。**它把一条 call-chain 解析成"按顺序、可直接复制到 Postman 的请求清单"**——URL、method、headers、body、期望响应都明示给用户;非 HTTP 入口(Scheduler / MQ / RPC)经用户同意后,由当前 Codex 会话添加临时 `/_smoke/trigger/{name}` HTTP 入口,让用户在 Postman 也能戳到。
...
向用户确认 让用户从 `.harness/call-chain/*.md` 列表中**单选**一条 slug。多 slug 没有合并价值——每条流程独立成单元,跑完再选下一个更清晰。
...
向用户确认,把非 HTTP 入口列出,三选一:

> 该 call-chain 有 N 个非 HTTP 入口(列出 entry-type + 类名),要不要添加临时 `/_smoke/trigger/{name}` HTTP 入口?
>
...
对每个被选"加"的入口,由当前 Codex 会话做窄范围代码改动,任务边界写死:
...
改动完成后,本 skill 把这些路径写入 `.harness/smoke-requests/{slug}/temp-controllers.md`,格式:
...
读取该文件,把所有临时 Controller 文件路径聚合,向用户确认 三选一:
...
> - **直接提交**:`git add {files} && git commit`(commit message 由用户提供或默认 `chore: smoke trigger endpoints for {slug}`)。**默认不 push**——push 是显著动作,需要用户在另一条 向用户确认 里再次确认目标 branch
...
- **不静默写文件**——临时 Controller 清理、调整后的请求覆盖,关键节点必须经用户 向用户确认 同意
