...
# 参考原 Claude Code 版的 tmux pane 管理方式；Codex 版通过初始 prompt 注入角色。
...
# Codex 版默认启动 codex CLI。HARNESS_CLI 可覆盖为完整命令前缀。
if [ -z "${HARNESS_CLI:-}" ] && ! command -v codex &>/dev/null; then
  echo "错误：codex CLI 未安装或不在 PATH 中"
  exit 1
fi
...
launch_agent_pane() {
  ...
  new_pane=$(tmux split-window -d $split_args -t "$target_pane" -P -F '#{pane_id}' \
    "export HARNESS_CONFIG='$config_path' HARNESS_AGENT_NAME='$agent' && cd '$PROJECT_DIR' && ${cli_cmd}")
