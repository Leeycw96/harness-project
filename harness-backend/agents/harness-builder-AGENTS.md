...
| **步骤** | 1. Read qa-feedback<br>2. 按 P0 → P1 → P2 顺序修**根因**(不修症状)<br>3. 涉及调用链路变更时同步 call-chain<br>4. 跑全量 mvn test(含 QA 补充的 `QA_*.java` 若有)<br>5. `complete_and_notify "harness-qa" "FIX_DONE | 修复完成,请重审 | artifact: ${output_dir}/qa-feedback-round-{N}.md" "${output_dir}/qa-feedback-round-{N}.md"` |
| **完成标准** | qa-feedback 列出的 P0/P1 全部根因修掉;mvn test 全绿。**通知 qa 后本 turn 立即停手**(STOP 边界),不输出用户面向文本 |
