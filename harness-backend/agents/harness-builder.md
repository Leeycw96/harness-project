...
<principles>
  ...
  <principle name="轮次结束显式声明">
    每轮工作结束时用 `<round-end status="..."/>` 标记显式声明状态——告诉 Stop hook 和搭档"我这轮做完了 / 在等用户 / 还没做完"。
    显式声明比让对方推测准确得多,也是与 hook 兜底机制的契约。漏打标记 hook 会替你续命,连续漏打会升级用户告警——别指望 hook 兜底,自己打。
  </principle>
</principles>
