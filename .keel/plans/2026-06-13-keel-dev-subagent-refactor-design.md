# Keel Dev Subagent Refactor Design

Date: 2026-06-13

## Background

`keel-dev` started as a long-running Builder + QA workflow. Production use
...
This refactor changes `keel-dev` into a Codex App only, subagent-only
...
- No user-adjustment phase in this refactor. After QA and CodeReview pass, the
  run ends. New changes should start a new plan/dev run.
...
`bin/keel` will become a Codex App deployment CLI. It will no longer accept
...
`codex-cli/` will be removed. The old `keel-solidity/` mode is outside the
new dev workflow and will also be removed from the deployable surface.
...
- `keel-builder`: creates scope, implements feature slices, fixes blocking
  feedback, and commits Builder changes.
- `keel-qa`: reviews scope and validates business behavior. It produces the
  business verification package and QA verdict.
- `keel-code-review`: reviews the Builder commit diff for correctness,
...
The design uses those semantics but implements a `keel-code-review` custom
agent because the UI `/review` command is not a stable internal stage API for
the Keel orchestrator.
...
`/keel-plan` remains the entry point for creating machine-readable XML
...
The output contract stays `.keel/plans/<name>.md` in the existing XML
...
- `README.md`
- `AGENTS.md`
- `common/scripts/keel-init.sh`
- `common/scripts/keel-common.sh`
- `keel-plan/skills/keel-plan/SKILL.md`
- `keel-dev/skills/keel-dev/SKILL.md`
- `keel-dev/agents/keel-builder.md`
- `keel-dev/agents/keel-builder-AGENTS.md`
- `keel-dev/agents/keel-qa.md`
- `keel-dev/agents/keel-qa-AGENTS.md`
- `keel-dev/agents/keel-code-review.md`
- `keel-dev/agents/keel-code-review-AGENTS.md`
- `keel-dev/agents/keel-builder.toml`
- `keel-dev/agents/keel-qa.toml`
- `keel-dev/agents/keel-code-review.toml`
...
- old dev smoke skill from the deployable surface; a future smoke workflow
