# Harness Plan HTML 审阅界面设计

## 背景与目标

Codex App 的 Harness Plan 当前生成两份相互关联的产物：XML 需求契约和 Markdown 代码改造方案。Backend 初始化脚本解析 XML，Builder 与 QA 同时消费两份计划；用户则需要在会话中审阅两份较长的完整草稿，容易产生阅读疲劳。

本次为 Codex App Harness Plan 增加一个桌面端、自包含的 HTML 审阅界面。HTML 用两个标签页呈现现有两份计划，并成为会话中的唯一完整审阅入口。XML 和 Markdown 继续作为 Agent 与脚本的唯一事实来源，现有 Backend 契约不变。

Claude Code runtime 仍维持当前单 XML 产物流程，不在本次范围内。

## 产物契约

Harness Plan 最终生成三份文件：

1. `.harness/plans/<name>.md`：XML 需求契约，供初始化脚本解析。
2. `.harness/plans/<name>-implementation.md`：Markdown 代码改造方案，供 Builder 和 QA 消费。
3. `.harness/plans/<name>.html`：面向用户的唯一完整审阅入口。

XML 中的 `<implementation-plan>` 声明保持不变，不引用 HTML。Backend、Builder 和 QA 不读取、解析或复制 HTML。HTML 是 Harness Plan 的必需产物；缺失时 Plan 不得报告完成或提示进入 Backend。

## 架构与组件

新增两个 Codex Harness Plan 内部组件：

- `harness-plan/skills/harness-plan/assets/plan-view-template.html`：自包含的桌面端页面模板，包含样式、标签页行为和安全渲染逻辑。
- `harness-plan/skills/harness-plan/scripts/render-plan-html.sh`：确定性渲染入口，只读消费 XML 和 Markdown，生成对应 HTML。

部署后的调用形式为：

```bash
bash .agents/skills/harness-plan/scripts/render-plan-html.sh \
...
XML 和 Markdown 始终是唯一事实来源。用户反馈导致内容变化时，Harness Plan 先更新相应源文件、重新运行现有契约校验，再重新生成 HTML；禁止只修改 HTML。

## Harness Plan 流程

Harness Plan 保留现有调研、澄清、技术决策和分段确认门禁，并调整最终审阅流程：
...
6. 用户最终确认后，报告三份文件路径，并提示选择 `/harness-backend-fast` 或 `/harness-backend`。
...
HTML 生成失败时保留已验证的 XML 与 Markdown。Harness Plan 针对失败原因重试一次；再次失败则报告明确错误，Plan 保持未完成，不提示进入 Backend。
...
- HTML 模板和渲染脚本存在；
- Harness Plan Skill 声明三份产物和 HTML 审阅流程；
...
README 更新 Codex Harness Plan 为“三文件、单审阅入口”，明确用户审阅 HTML，Backend 仍只消费 XML 与 Markdown。部署目录示例同步加入新增模板和脚本。

现有双产物计划仍可直接交给 Backend，不要求补生成 HTML。只有运行新版 Harness Plan 的新计划必须包含 HTML。Claude Code runtime、旧计划和现有 Backend run 不迁移。

## Out of Scope

- 用 HTML 替代 XML 或 Markdown；
- 修改 Backend、Builder 或 QA 的输入契约；
- 修改 Claude Code Harness Plan；
- 移动端适配；
