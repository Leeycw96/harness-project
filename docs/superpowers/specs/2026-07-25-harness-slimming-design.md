...
Harness 当前通过 Skill、Agent 手册、磁盘状态和多阶段 workflow 为模型补充需求澄清、实现、验收、恢复等能力。随着 GPT-5.6 SOL、Kimi K3 等强模型的能力边界扩大，现有大量教程式说明、重复协议和固定 Agent 调度已不再全部必要。

当前仓库同时维护 Codex App 和 Claude Code 两套 runtime。审计结果如下：

- Codex runtime 约 2,818 行，Claude Code runtime 约 2,834 行。
- `harness-backend`、`harness-backend-fast`、`harness-backend-fix` 重复描述初始化、预检、生命周期、进度恢复和 artifact 规则。
...
保留 `harness-backend-coding-rules.md`。以下要求属于 Harness 的工程政策，不能交给模型自行推断：
...
- 原需求内问题通过新的 plan/backend run 处理。
- 范围变化或新需求重新运行 `/harness-plan`。
- README 和 full/fast 完成提示不再推荐 `/harness-backend-fix`。

## 删除范围

第一期删除两套 runtime 中的：

- `harness-backend-fix` Skill
- `harness-feedback-triage` Agent
...
部署 manifest 继续按现有规则删除目标项目中上一版由 Harness 管理、但新版源码已不存在的文件。
...
```bash
tmpdir=$(mktemp -d)
bin/harness backend --codex "$tmpdir"
bin/harness backend --claude-code "$tmpdir"
rm -rf "$tmpdir"
git status --short
```
...
- 第一期直接删除独立 `/harness-backend-fix` 和 FeedbackTriage。
