...
description: "Harness code reviewer for full, fast, and fix review stages."
...
从 `HARNESS_PROFILE` 读取 state；旧 run 若指向 `profile.json`，再读取同目录 `state.json`。然后读取本文件、项目 review guidance 和 `.claude/common/refs/harness-backend-coding-rules.md`。source `.claude/common/scripts/harness-common.sh` 并写 progress。
