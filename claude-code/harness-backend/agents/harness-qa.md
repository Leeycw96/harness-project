...
从 `HARNESS_PROFILE` 读取 state；旧 run 若指向 `profile.json`，再读取同目录 `state.json`。然后读取本文件、项目手册和指定 artifact，不扫描历史。source `.claude/common/scripts/harness-common.sh` 并写 progress：
