...
`AskUserQuestion` 让用户从 `.harness/call-chain/*.md` 列表中**单选**一条 slug。多 slug 没有合并价值——每条流程独立成单元,跑完再选下一个更清晰。
...
`AskUserQuestion`,把非 HTTP 入口列出,三选一:
...
5. `AskUserQuestion`:「继续后续 step / 中止本次冒烟」
...
读取该文件,把所有临时 Controller 文件路径聚合,`AskUserQuestion` 三选一:
...
> - **直接提交**:`git add {files} && git commit`(commit message 由用户提供或默认 `chore: smoke trigger endpoints for {slug}`)。**默认不 push**——push 是显著动作,需要用户在另一条 `AskUserQuestion` 里再次确认目标 branch
...
- **不静默写文件**——临时 Controller 清理、调整后的请求覆盖,关键节点必须经用户 `AskUserQuestion` 同意
