#!/usr/bin/env bash
# Keel Codex App common helpers.
# Disk contract: state.json + progress.tsv. KEEL_PROFILE is the compatible state path variable.

SCRIPT_PATH="${BASH_SOURCE[0]-$0}"
SCRIPT_DIR="$(cd "$(dirname "$SCRIPT_PATH")" && pwd)"
PROJECT_DIR="${PROJECT_DIR:-$(cd "$SCRIPT_DIR/../../.." && pwd)}"

KEEL_PROFILE="${KEEL_PROFILE:-$PROJECT_DIR/.keel/state.json}"
...
_keel_profile_value() {
  local expr="$1"
  _keel_require_jq || return 1
  [ -f "$KEEL_PROFILE" ] || {
    echo "错误: KEEL_PROFILE 不存在: $KEEL_PROFILE" >&2
    return 1
  }
  jq -r "$expr // empty" "$KEEL_PROFILE"
}

keel_output_dir() {
  _keel_profile_value '.output_dir'
}

keel_state_file() {
  local output_dir
  case "$KEEL_PROFILE" in
    state.json|*/state.json)
      printf '%s\n' "$KEEL_PROFILE"
      return
      ;;
  esac
  output_dir="$(keel_output_dir)" || return 1
...
keel_progress_file() {
  ...
  configured="$(_keel_profile_value '.progress.events')" || return 1
...
append_progress_event() {
  ...
  events="$(keel_progress_file)" || return 1
...
update_progress() {
  ...
  progress_file="$(keel_progress_file)" || return 1

  case "$agent" in
    keel-builder|builder) display_agent="keel-builder" ;;
    keel-qa|qa) display_agent="keel-qa" ;;
    keel-call-chain|call-chain|call_chain) display_agent="keel-call-chain" ;;
    *) display_agent="$agent" ;;
  esac
...
record_call_chain_result() {
  ...
  artifact="$(_keel_profile_value '.artifacts.call_chain_review')" || return 1
  ts="$(date +%FT%T%z)"
  _keel_state_jq \
